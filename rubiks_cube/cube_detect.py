    ...
    if min_area < area < max_area:                       # filter out too small and too large contours (areas)                                              # that contour isn't  
        ...
        if edges_delta < square_ratio and axes_ratio > rhombus_ratio:      # check if the contour looks like a square
            ...
            if abs(inclination)<max_inclination:                  # filter out when the (1st) facelets (cube faces) are too inclined
                ...
                #중심축 찍기
                # img=cv2.line(frame, (cX, cY), (cX, cY), (0,0,255), 5)
                # cv2.imshow('test', img)


                                    # do_flag=0
                    # for one_facelets in facelets:
                    #     point_len=math.sqrt((facelets_tmp['cx']-one_facelets['cx'])**2-(facelets_tmp['cy']-one_facelets['cy'])**2)
                    #     if point_len<5:
                    #         do_flag=1
                    
                    # if do_flag==0:
                    #     facelets.append(facelets_tmp)
                ...
                if len(facelets)>=7:
                    ...
                    for k in range(3):
                        ...
                        for j in range(3):
                            ...
                            #img=cv2.line(frame, (int(new_x.item()), int(new_y.item())), (int(new_x.item()), int(new_y.item())), (0,0,255), 5)
                            #cv2.imshow('test', img)
...
def read_color(facelets, candidates, BGR_mean, H_mean, wait=20, index=0):
    ...
    for facelet in facelets:                              # iteration over the 9 facelets just detedcted
        ...
        bgr_mean_sq = average_color(frame, int(facelet[0]), int(facelet[1]))  # color is averaged with sqr sum os squares
...
def start_up():
    ...
    #if device == 'laptop':                            # device dependent startup
    if True:
#         if debug:
#             %matplotlib inline
#             import matplotlib.pyplot as plt           # matplotlib is used to plot camera images to the terminal while debugging
        
        global clear_output, first_cycle, plt, k_kernel, d_iterations, e_iterations, facelets_in_width         # function used also elsewhere
        ...
        if hierarchy is not None:              # analyze the contours in case these are previously retrieved
            ...
            for component in zip(contours, hierarchy):                         # each contour is analyzed   
                ...
                if len(ax_lists)==9:                             # case having 9 contours, within the right distance (= a complete cube face)
                    ...
                    print(ax_lists)
                    read_color(ax_lists, candidates, BGR_mean, H_mean)
                    ax_lists=[]
...
if __name__ == "__main__":
    ...
    #if device == 'laptop':                      # case for the robot
    if True:
