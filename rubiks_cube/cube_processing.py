...
def robot_camera_warmup():
    ...
    if device=='laptop':
        ...
        if screen:
#             print(f'\na_gain_list: {a_gain_list}')
#             print(f'\nd_gain_list: {d_gain_list}')
#             print(f'\nawb_blue_list: {awb_blue_list}')
#             print(f'\nawb_red_list: {awb_red_list}')
#             print(f'\ntime: {t_list}')
#             print(f'\ndatapoints: {len(t_list)}')
...
def robot_consistent_camera_images():
    """
    Picamera is left in Auto mode for Exposure and AWB, untill the first 9 facelets (1st side) are detected.
    For consistent color detection on the following sides, those 2 parameters are retrieved from PiCamera, and set back
    to PiCamera as manual parametes.
    This prevent the PiCamera to adjust the AWB and Exposition while reading the following 5 cube faces (sides)
    """
    ...
    if device=='laptop':
        ...
        a_gain = camera.analog_gain                         # read picamera setting (used at 1st cube side) analog gain
        time.sleep(0.01)
        d_gain = camera.digital_gain                        # read picamera setting (used at 1st cube side) digital gain
        time.sleep(0.01)
        awb_gain = camera.awb_gains                         # read picamera setting (used at 1st cube side) AutoWhiteBalance gain
        camera.awb_mode = 'off'                             # sets white balance off
        time.sleep(0.01)
        camera.awb_gains = awb_gain                         # sets AWB gain used on side 1, for the remaining sides
        time.sleep(0.01)
        camera_set_gains.set_analog_gain(camera, a_gain)    # sets analog gain used on side 1, for the remaining sides
        time.sleep(0.01)
        camera_set_gains.set_digital_gain(camera, d_gain)   # sets digital gain used on side 1, for the remaining sides
        time.sleep(0.01)
...
def get_facelets(contour, hierarchy):
    ...
    if device=='laptop': 
        ...
        min_area = 700                 # min area limit for a single facelet's contour (at robot the picamera is much closer to the cube)
        max_area = 5000                # max area limit for a single facelet's contour (at robot the picamera is much closer to the cube)
        square_ratio=2                 # considered like a square when min side/max side < square_ratio: Extremely permissive at robot
        rhombus_ratio=0                # considered like a square when diagonal1/diagonal2 > rhombus_ratio: Extremely permissive at robot
        max_inclination=20             # max inclination limit for the 1st facelet vs horizon
         
    area = cv2.contourArea(contour)                        # area of each passed contour is retrieved
    
    if device=='laptop' and hierarchy>=0:                  # when laptop camera and contour is an outermost (it has inner contour/contours) 
...
def robot_next_side(side):
    ...
    if device == 'laptop':
        ...
        if side in range(4):                         # first 4 sides (3 sides apart the one already in front of the camera)
            servo.flip(1, not robot_stop, align)     # are areached by simply flipping the cube
        if side == 4 :                               # at side 4 is needed to change approach to show side 5 to the camera
            if robot_stop == False:                  # check for robot_stop boolean
                servo.spin(1)                        # one spin (90 deg) is applied to the cube
                servo.flip(1, not robot_stop, align) # one flip is applied to the cube to show the side 5 to the camera
        if side == 5 :                               # when side5 is in front of the camera 
            servo.flip(2, not robot_stop, align)     # at side 5 are needed two flips to show side 6 to the camera
        ...
        # after reading the full cube is positioned to initial position/orientation, to apply Kociemba solution     
            ...
            if robot_stop == False:                   # global variable robot_stop is checked before each movement  
                servo.flip(1, not robot_stop, align)  # 1 cube flip
            if robot_stop == False:                   # global variable robot_stop is checked before each movement     
                servo.spin(-1)                        # 1 ccw spin
            if robot_stop == False:                   # global variable robot_stop is checked before each movement  
                servo.flip(1, not robot_stop, align)  # 1 cube flip
            if robot_stop == False:                   # global variable robot_stop is checked before each movement  
...
def quit_func():
    ...
    if device == 'laptop':
        ...
        if screen:
            ...
            print('\nRequested to quit: AF_cube_robot_noVideo.py\n')
        ...
        for i in range(2):
            ...
            servo.servo_start_positions()               # top cover ancd cube lifter set to start position
            time.sleep(0.3)                             # time to ensure servos are at their start positions
            servo.servo_off()                           # de-energizes the servos
            servo.motor_off()                           # de-energizes the motor

        try:
            robot_load_status.cancel()                  # reset the "loading" status at robot_display1
        except:
            if debug:
                print(f'Issue on closing robot_load_status thread')
            pass
        ...
        try:
            ...
            if debug:
                print(f'Issue on closing robot_time_display thread')
            pass
        ...
        try:
            ...
            if debug:
                print(f'Issue on closing robot_reading_status_display thread')
            pass
        ...
        try:
            ...
            if debug:
                print(f'Issue on closing robot_press_feedback thread')
            pass
        ...
        try:
            ...
            print('Issues at camera closing while quitting')
            pass
...
def start_up():
    ...
    global font, fontScale, fontColor, lineType, camera, width, height, device, sv, quitting
    ...
    device = "Rpi"           # verify if the script is running at Rpi (robot), if not then it is the laptop
    print("hardware check, ", str(device))
    side=0
    
    if device == 'Rpi':                            # device dependent startup
#         if debug:
#             %matplotlib inline
#             import matplotlib.pyplot as plt           # matplotlib is used to plot camera images to the terminal while debugging
        
        global first_cycle, plt, k_kernel, d_iterations, e_iterations, facelets_in_width         # function used also elsewhere
        
        import solver as sv                           # Kociemba solver
        
#         clear_terminal(device)                        # cleares the terminal
        first_cycle=True                              # variable to be used only at first program loop
        camera, rawCapture, width, height = webcam()              # webcam relevant info are returned after cropping, resizing, etc
...
def cubeAF():
    ...
    while quitting == False:   # substantially the main loop, it can be interrupted by quit_func() 
        ...
        if hierarchy is not None:              # analyze the contours in case these are previously retrieved
            ...
            if device=='Rpi':
                if timeout==True:                        # in case of reached timeout or stop_button pressed
            ...
            for component in zip(contours, hierarchy):                         # each contour is analyzed   
                ...
                if len(facelets)==9:                             # case having 9 contours, within the right distance (= a complete cube face)
                    if device == 'Rpi' and side ==1:             # after acquiring the first side the picamera settings are changed
                        robot_consistent_camera_images()         # PiCamera is set to capture consistent images
                    ...
                    if side == 6:   # last cube's face is acquired   
                        ...
                        if device == 'laptop':
                            ...
                            window_for_cube_solving(solution_Text, w, h, side, frame)  # image stream while user manually solves the cube (laptop mode)
                            side = 0
                            break                        
                        
                        elif device == 'Rpi':                # specific for robot, when the last face has been completely detected
                            # function related to cube solving via the robot
                            robot_solve_cube(fixWindPos, screen, device, frame, faces, edge, cube_status, cube_color_sequence,
                                                kociemba_facelets_BGR_mean, font, fontScale, lineType, show_time, timestamp,
                                                solution, solution_Text, color_detection_winner, cube_status_string, BGR_mean,
                                                HSV_detected) 
                        
                        time.sleep(10)
                        if device == 'Rpi' and robot_stop==False:       # specific part for the robot
                ...
                #########################################################################################################################
                #### below part relates to the way to interrupt the CV2 windows, at different moment in time of the script execution ####
                #########################################################################################################################
            ...
            if screen:
                ...
                if key == 27 & 0xFF:  # ESC button
                    quit_func()       # quit function is called
                    break             # for loop is interrupted (the while loop havieng some contour)
        ...
        if screen:
            ...
            if key == 27 & 0xFF:    # ESC button
                quit_func()         # quit function is called
                break               # while loop is interrupted (the while loop without any contour)

        if device=='Rpi':
            if timeout==True:   # timeout or motor_motor check for Rpi
...
if __name__ == "__main__":
    ...
    """
    global side, screen, fixWindPos
    debug=False                                 # flag to enable/disable the debug related prints
    screen=False                                # flag to diversify two scripts in Rpi, having or not data sharing to a screen
